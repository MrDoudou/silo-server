...
const REFRESH_REASON_LABELS: Record<string, string> = {
  episode_incomplete: "pages.admin_task_detail.episode_incomplete",
  stale_provider_id: "pages.admin_task_detail.stale_provider_id",
  refresh_failure: "pages.admin_task_detail.refresh_failure",
  core_metadata_incomplete: "pages.admin_task_detail.core_metadata_incomplete",
  trailers_requested: "pages.admin_task_detail.trailers_requested",
};
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="surface-panel rounded-2xl border-0 p-4">
          <div className="text-muted-foreground text-xs tracking-[0.16em] uppercase">
            {tr("pages.admin_task_detail.next_claim_timeout")}
          </div>
          <div className="mt-2 text-sm font-medium">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="surface-panel rounded-2xl border-0 p-4">
          <h3 className="text-sm font-medium">{tr("pages.admin_task_detail.reason_breakdown")}</h3>
          <div className="mt-3 flex flex-wrap gap-2">
            {reasonCounts.length === 0 && (
              <span className="text-muted-foreground text-sm">
                {tr("pages.admin_task_detail.no_queued_debt")}
              </span>
            )}
            {reasonCounts.map((entry) => (
              <Badge key={entry.reason} variant="secondary">
                {tr(REFRESH_REASON_LABELS[entry.reason] ?? entry.reason)}: {entry.count}
              </Badge>
            ))}
          </div>
            ...
            {metrics.attempt_buckets.map((bucket) => (
              <div key={bucket.label} className="bg-muted/30 rounded-xl p-3">
                <div className="text-muted-foreground text-xs">
                  {tr("pages.admin_task_detail.attempts")} {bucket.label}
                </div>
        ...
        <div className="surface-panel rounded-2xl border-0 p-4">
          <h3 className="text-sm font-medium">{tr("pages.admin_task_detail.recent_errors")}</h3>
          <div className="mt-3 space-y-3">
            {metrics.recent_errors.length === 0 && (
              <p className="text-muted-foreground text-sm">
                {tr("pages.admin_task_detail.no_recent_queue_errors")}
              </p>
            )}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-muted-foreground text-xs">
                      {entry.type || tr("pages.admin_task_detail.item")}{" "}
                      {tr("pages.admin_task_detail.attempts_0e7d7f95")} {entry.attempt_count}
                    </div>
                  </div>
      ...
      <div className="surface-panel overflow-hidden rounded-2xl border-0">
        <div className="border-border border-b px-4 py-3">
          <h3 className="text-sm font-medium">{tr("pages.admin_task_detail.due_samples")}</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-border bg-muted/40 border-b">
              <th className="px-4 py-2 text-left font-medium">
                {tr("pages.admin_task_detail.item_ecdda59a")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {tr("pages.admin_task_detail.next_refresh")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {tr("pages.admin_task_detail.attempts")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {tr("pages.admin_task_detail.last_attempt")}
              </th>
            </tr>
          </thead>
          <tbody>
            {metrics.due_samples.length === 0 && (
              <tr>
                <td colSpan={4} className="text-muted-foreground px-4 py-4 text-center">
                  {tr("pages.admin_task_detail.no_due_items_right_now")}
                </td>
              </tr>
            )}
...
const TRIGGER_TYPES: { value: TriggerType; label: string }[] = [
  {
    value: "interval",
    get label() {
      return tr("pages.admin_task_detail.interval");
    },
  },
  {
    value: "daily",
    get label() {
      return tr("pages.admin_task_detail.daily");
    },
  },
  {
    value: "weekly",
    get label() {
      return tr("pages.admin_task_detail.weekly");
    },
  },
  {
    value: "startup",
    get label() {
      return tr("pages.admin_task_detail.on_startup");
    },
  },
];
      ...
      <div className="flex-1 space-y-2">
        ...
        {trigger.type === "interval" && (
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="seconds">{tr("pages.admin_task_detail.seconds")}</SelectItem>
                <SelectItem value="minutes">{tr("pages.admin_task_detail.minutes")}</SelectItem>
                <SelectItem value="hours">{tr("pages.admin_task_detail.hours")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <label className="text-muted-foreground text-xs whitespace-nowrap">
            {tr("pages.admin_task_detail.max_runtime")}
          </label>
          <Input
            type="number"
            min={0}
            className="w-full sm:w-[100px]"
            placeholder={tr("pages.admin_task_detail.none")}
          ...
          <span className="text-muted-foreground text-xs">
            {tr("pages.admin_task_detail.minutes_be2e2bb6")}
          </span>
        </div>
      </div>
...
export default function AdminTaskDetail() {
  ...
  if (isLoading || !task) {
    return (
      <p className="page-shell text-muted-foreground py-8 text-sm">
        {tr("pages.admin_task_detail.loading")}
      </p>
    );
  }
  ...
  return (
    <div className="page-shell space-y-6 py-4 sm:py-6">
      <nav
        aria-label={tr("pages.admin_task_detail.breadcrumb")}
        className="text-muted-foreground flex items-center gap-1.5 text-sm"
      >
        <Link to="/admin" className="hover:text-foreground transition-colors">
          {tr("pages.admin_task_detail.admin")}
        </Link>
        <ChevronRight className="h-3.5 w-3.5" />
        <Link to="/admin/tasks" className="hover:text-foreground transition-colors">
          {tr("pages.admin_task_detail.scheduled_tasks")}
        </Link>
        <ChevronRight className="h-3.5 w-3.5" />
      ...
      <div className="page-header gap-5">
        ...
        {isRunning ? (
          ...
          >
            ...
            {tr("pages.admin_task_detail.run_now")}
          </Button>
        )}
      </div>
      ...
      {task.key === "scan_libraries" && (
        <div className="surface-panel-subtle rounded-xl p-4 text-sm leading-relaxed">
          <span className="text-muted-foreground">
            {tr("pages.admin_task_detail.this_task_history_records_how_long_it_took_to_queue")}{" "}
          </span>
          <Link to="/admin/libraries" className="text-foreground hover:text-primary font-medium">
            {tr("pages.admin_task_detail.admin_libraries")}
          </Link>
          <span className="text-muted-foreground">
            {" "}
            {tr("pages.admin_task_detail.and_server_activity")}
          </span>
        </div>
      )}
          ...
          <div className="text-muted-foreground flex items-center justify-between gap-3 text-sm">
            <p className="min-w-0 truncate">
              {task.state === "cancelling"
                ? tr("pages.admin_task_detail.cancelling")
                : task.progress_message || tr("pages.admin_task_detail.running")}
            </p>
      ...
      {task.key === "refresh_metadata" && metrics && (
        <div className="space-y-3">
          <h2 className="text-lg font-medium tracking-tight">
            {tr("pages.admin_task_detail.queue_health")}
          </h2>
      ...
      <div className="space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-lg font-medium tracking-tight">
            {tr("pages.admin_task_detail.schedule")}
          </h2>
          {!task.manual_only && !editing && (
            <Button variant="outline" size="sm" onClick={startEditing}>
              {tr("pages.admin_task_detail.edit_schedule")}
            </Button>
          )}
        </div>
        ...
        {task.manual_only ? (
          <div className="surface-panel-subtle rounded-xl p-4 text-sm">
            <p className="text-muted-foreground">
              {tr("pages.admin_task_detail.manual_only_this_task_runs_only_when_you_select_run")}
            </p>
          </div>
        ) : !editing ? (
          <div className="surface-panel overflow-hidden rounded-2xl border-0">
            {task.triggers.length === 0 && (
              <p className="text-muted-foreground p-4 text-sm">
                {tr("pages.admin_task_detail.no_triggers_configured")}
              </p>
            )}
          ...
          <div className="surface-panel rounded-2xl border-0 p-4">
            <div className="space-y-3">
              ...
              <Button variant="outline" size="sm" onClick={addTrigger}>
                ...
                {tr("pages.admin_task_detail.add_trigger")}
              </Button>
      ...
      <div className="space-y-3">
        <h2 className="text-lg font-medium tracking-tight">
          {tr("pages.admin_task_detail.execution_history")}
        </h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-border bg-muted/50 border-b">
                <th className="px-4 py-2 text-left font-medium">
                  {tr("pages.admin_task_detail.started")}
                </th>
                <th className="px-4 py-2 text-left font-medium">
                  {tr("pages.admin_task_detail.duration")}
                </th>
                <th className="px-4 py-2 text-left font-medium">
                  {tr("pages.admin_task_detail.status")}
                </th>
                <th className="px-4 py-2 text-left font-medium">
                  {tr("pages.admin_task_detail.error")}
                </th>
              </tr>
            </thead>
            <tbody>
              {(!history || history.length === 0) && (
                <tr>
                  <td colSpan={4} className="text-muted-foreground px-4 py-4 text-center">
                    {tr("pages.admin_task_detail.no_execution_history")}
                  </td>
                </tr>
              )}
              {history?.map((result) => (
