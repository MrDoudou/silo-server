  ...
  if (statsQuery.error) {
    return (
      <div className="surface-panel flex h-full items-center rounded-2xl border-0 px-4">
        <SectionError
          message={tr(
            "components.admin.dashboard.widgets.watch_providers_widget.failed_to_load_watch_provider_activity",
          )}
        />
      </div>
    );
    ...
    return (
      <div className="surface-panel flex h-full min-h-14 items-center gap-3 rounded-2xl border-0 px-4 py-3">
        <span className="text-muted-foreground text-sm">
          {tr(
            "components.admin.dashboard.widgets.watch_providers_widget.no_watch_providers_configured",
          )}
        </span>
      </div>
    );
    ...
    <div className="flex min-h-14 flex-shrink-0 flex-wrap items-center gap-x-3 gap-y-1 px-4 py-3">
      ...
      <span className="text-muted-foreground min-w-0 flex-1 text-sm">
        ...
        {profiles === 0 ? (
          <>
            {" · "}
            {provider.registered
              ? tr("components.admin.dashboard.widgets.watch_providers_widget.not_connected")
              : tr("components.admin.dashboard.widgets.watch_providers_widget.not_installed")}
          </>
        ) : (
      ...
      {errors > 0 ? (
        <span className="bg-destructive/10 text-destructive flex-shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium tabular-nums">
          ...
          {errors === 1
            ? tr("components.admin.dashboard.widgets.watch_providers_widget.error")
            : tr("components.admin.dashboard.widgets.watch_providers_widget.errors")}
        </span>
      ) : null}
      <Link
      ...
      >
        {tr("components.admin.dashboard.widgets.watch_providers_widget.manage")}
      </Link>
    </div>
  );
}
...
function ProviderActivity({ provider }: { provider: WatchProviderStats }) {
  ...
  return (
    <>
      ...
      {profiles === 1
        ? tr("components.admin.dashboard.widgets.watch_providers_widget.profile")
        : tr("components.admin.dashboard.widgets.watch_providers_widget.profiles")}
      {tr("components.admin.dashboard.widgets.watch_providers_widget.synced")}
      {lastSync}
      <span className="text-border/70 mx-1.5">|</span>
      {tr("components.admin.dashboard.widgets.watch_providers_widget.value_24h")}
      <span className="text-foreground font-medium">
        {provider.imported_watched_24h.toLocaleString()}
      </span>
      {tr("components.admin.dashboard.widgets.watch_providers_widget.in")}
      {provider.exporting ? (
        <>
          {" / "}
          <span className="text-foreground font-medium">
          ...
          {tr("components.admin.dashboard.widgets.watch_providers_widget.out")}
        </>
      ) : null}
      {provider.scrobbling && provider.open_scrobbles > 0 ? (
        <>
          ...
          {tr("components.admin.dashboard.widgets.watch_providers_widget.scrobbling")}
        </>
      ) : null}
    </>
  );
