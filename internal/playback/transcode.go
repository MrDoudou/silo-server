...
// setThrottlePaused mirrors the throttler's state into event diagnostics.
func (s *TranscodeSession) setThrottlePaused(paused bool) {
	s.mu.Lock()
...
// logFFmpegThrottleEvent records a small state transition without duplicating
// the command snapshot stored by ConfigureThrottler.
func (s *TranscodeSession) logFFmpegThrottleEvent(
	ctx context.Context,
	message string,
	paused bool,
	gapSeconds int,
