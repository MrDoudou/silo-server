...
func (t *TranscodeThrottler) CheckOnce() {
	...
	if progressPredatesGeneration(progress) {
		...
		t.mu.Unlock()
		if event != "" {
			t.session.setThrottlePaused(false)
			t.session.logFFmpegThrottleEvent(context.Background(), event, false, gap)
		}
		return
	}
	...
	t.mu.Unlock()
	if event != "" {
		t.session.setThrottlePaused(paused)
		t.session.logFFmpegThrottleEvent(context.Background(), event, paused, gap)
	}
}
