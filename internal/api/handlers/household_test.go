...
func TestAuthorizeHouseholdProfileTarget(t *testing.T) {
	ctx := context.Background()
	store := newHouseholdTestStore(t)
	if err := store.CreateProfile(ctx, userstore.Profile{
		ID: "primary", Name: "Sam", IsPrimary: true,
	...
	t.Run("own profile allowed", func(t *testing.T) {
		ok, err := authorizeHouseholdProfileTarget(
			householdRequest("child", false, ""), store, nil, tokens, "child")
		if err != nil || !ok {
			t.Fatalf("own profile = (%v, %v), want (true, nil)", ok, err)
		}
	})

	t.Run("sibling requires household management", func(t *testing.T) {
		ok, err := authorizeHouseholdProfileTarget(
			householdRequest("child", false, ""), store, nil, tokens, "primary")
		if err != nil || ok {
			t.Fatalf("sibling = (%v, %v), want (false, nil)", ok, err)
		}
	})

	t.Run("primary may target sibling", func(t *testing.T) {
		ok, err := authorizeHouseholdProfileTarget(
			householdRequest("primary", false, ""), store, nil, tokens, "child")
		if err != nil || !ok {
			t.Fatalf("primary targeting sibling = (%v, %v), want (true, nil)", ok, err)
		}
	})

	t.Run("no active profile cannot widen", func(t *testing.T) {
		ok, err := authorizeHouseholdProfileTarget(
			householdRequest("", false, ""), store, nil, tokens, "child")
		if err != nil || ok {
			t.Fatalf("no profile = (%v, %v), want (false, nil)", ok, err)
		}
	})
