...
Plex history import now passes the advertised addresses for the selected server to Silo. The server tries a bounded set of those connections until one can return the library list, then uses that working address for the rest of the import. Existing clients that send only one Plex URL keep working unchanged.
